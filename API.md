...
`MEDIA_DIR/<userId>/` и раздаются по `MEDIA_BASE_URL` — по умолчанию это
`/api/media-file`, то есть файлы идут через то же проксирование, что и API,
и отдельный `location` в Nginx не нужен. Исторический префикс `/media/`
backend тоже отдаёт, поэтому старые ссылки продолжают работать.

В профиле помещается до **5 фото** и **одно видео-интро**: при превышении
приходит `400` с понятным текстом. Первое фото автоматически становится
главным — именно с ним сверяется верификация. Лимит: 30 загрузок в час.

### POST /api/spaces/cover

Обложка сообщества (multipart, поле `file`): только фото до 8 МБ, ответ
`{ url }`. В `profile_media` такой файл не попадает. Лимит: 20 в час.
`coverUrl` при создании сообщества принимает либо путь внутри сайта
(`/api/media-file/...`, `/media/...`, `/assets/...`), либо `https://`-ссылку.
...
Web Push по стандарту VAPID, без сторонних сервисов. Все запросы требуют
авторизации. Приватный ключ наружу не отдаётся никогда.

| Метод и путь | Назначение |
| --- | --- |
| `GET /api/push/public-key` | `{ enabled, publicKey }` — публичный VAPID-ключ. При пустых ключах в `.env` `enabled: false`, и фронтенд просто не предлагает включить пуши. |
| `POST /api/push/subscribe` | Тело — объект подписки браузера (`endpoint`, `keys.p256dh`, `keys.auth`). Upsert по `endpoint`, у одного пользователя может быть несколько устройств. Лимит 30/час. |
| `POST /api/push/unsubscribe` | `{ endpoint }` — удаляет подписку этого устройства. Лимит 60/час. |

Пуш отправляется вместе с записью в `notifications`:

- новое сообщение в личном чате (если получатель не держит диалог открытым);
- новая встреча, созданная организатором сообщества — всем участникам, кроме него;
- взаимный лайк (совпадение).

Учитываются переключатели типов уведомлений (`messages`, `spaces`, `matches`),
демо-профили (`is_seed`) в рассылку не попадают. Подписка, вернувшая 404/410
(Gone), удаляется из базы как невалидная.
