...
`MEDIA_DIR/<userId>/` и раздаются по `MEDIA_BASE_URL` (backend отдаёт этот
путь сам; в проде его же можно перехватить Nginx). Первое фото
...
(`/media/...`, `/assets/...`), либо `https://`-ссылку.
