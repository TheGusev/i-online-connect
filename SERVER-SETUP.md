...
Команда применяет и миграцию `014_notification_center.sql` с индексом истории
уведомлений. После обновления отдельная ручная команда для неё не требуется.

Проверка: `curl -s http://127.0.0.1:4000/api/health` → `{"ok":true,...}`.

Разовая уборка после обновления (миграция 011): фото, которые раньше попадали из
формы объявления в галерею профиля, переносятся в отдельное хранилище.

```bash
cd server
npm run split-listing-media -- --dry   # посмотреть, что будет перенесено
npm run split-listing-media            # перенести
```
...
> **Если после обновления новые функции отвечают 400/404** (например, голосовые
> сообщения стабильно получают 400 за миллисекунды) — почти всегда это значит,
> что на сервере работает старая сборка backend. Обязательная тройка команд
> после `rsync`: `npm run build` (без неё PM2 крутит старый `dist/`), затем
> `npm run migrate` (без новых миграций код падает на отсутствующих колонках),
> затем `pm2 reload ya-online-api`. Текст ошибки в ответе сервера называет
> конкретное поле — смотрите его в интерфейсе рядом с неотправленным сообщением.
