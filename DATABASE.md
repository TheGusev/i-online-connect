...
## 014_notification_center.sql — центр уведомлений

Составной индекс `notifications_user_kind_created_idx` ускоряет историю по
пользователю, фильтрацию по типу и cursor-пагинацию без пропусков при одинаковом
времени создания.

## 015–016 — голосовые и защита от дублей

`015_voice_kind.sql` добавляет значение `voice` к `message_kind` отдельной
транзакцией. `016_voice_messages.sql` добавляет к `messages` клиентский UUID,
URL, MIME-тип и длительность аудио. Частичный уникальный индекс по диалогу,
автору и клиентскому UUID делает повторный запрос идемпотентным.

## 017 — правка и удаление сообщений

`017_message_edits.sql` добавляет в `messages` колонки `edited_at` и
`deleted_at`. Удалённое сообщение остаётся строкой (чтобы порядок и курсоры
пагинации не ломались), но текст и медиа стираются, а проверка
`messages_voice_payload_check` пересоздаётся так, чтобы не срабатывать для
удалённых голосовых.
